...
const TestimonialCard: React.FC<TestimonialCardProps> = ({ cardProps, iconBadge, title, description, containerContentProps }) => {
    return (
        <Box pos={"relative"} pt={"22px"}>
            ...
            <Card shadow={"sm"} radius={"md"} p={"md"} bg={"dark.5"} withBorder {...cardProps}>
                <Text ms={"80px"} fz={12} fw={500} c={"gray.6"} {...title}>{title.text}</Text>
